...
func PassWordLogin(c *gin.Context) {
	...
	if ok := store.Verify(passwordLoginForm.CaptchaId, passwordLoginForm.Captcha, false); !ok {
		// 获取存储中的验证码值
		storedCaptchaValue := store.Get(passwordLoginForm.CaptchaId, false)

		// 打印详细的调试信息
		zap.S().Errorf("验证码验证失败。请求的验证码ID: %s, 请求的验证码值: %s, 存储的验证码值: %s",
			passwordLoginForm.CaptchaId, passwordLoginForm.Captcha, storedCaptchaValue)
